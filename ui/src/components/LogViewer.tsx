...
export function LogViewer({
  ...
  const storyHashSet = useMemo(() => {
    ...
    const hashes =
      activeStory?.entries.map((e) => e.hash).filter((h): h is string => !!h) ||
      [];
  ...
  const handleScroll = useCallback(() => {
    // This is called by Virtuoso's onScroll - we'll check atTopStateChange instead
  }, []);

  // Handle atTop state changes from Virtuoso
  const handleAtTopStateChange = useCallback(
    (atTop: boolean) => {
      isScrolledRef.current = !atTop;
      // If scrolled to top, show any buffered logs
      if (atTop && newLogsCount > 0) {
  ...
  useEffect(() => {
    if (
      jumpToNextError === undefined ||
      jumpToNextError === 0 ||
      errorIndices.length === 0
    )
      return;
    const nextIndex =
      currentErrorIndex < 0 ? 0 : (currentErrorIndex + 1) % errorIndices.length;
    setCurrentErrorIndex(nextIndex);
    virtuosoRef.current?.scrollToIndex({
      index: errorIndices[nextIndex],
      align: "center",
      behavior: "smooth",
    });
  }, [jumpToNextError]);
  ...
  useEffect(() => {
    if (
      jumpToPrevError === undefined ||
      jumpToPrevError === 0 ||
      errorIndices.length === 0
    )
      return;
    const prevIndex =
      currentErrorIndex <= 0 ? errorIndices.length - 1 : currentErrorIndex - 1;
    setCurrentErrorIndex(prevIndex);
    virtuosoRef.current?.scrollToIndex({
      index: errorIndices[prevIndex],
      align: "center",
      behavior: "smooth",
    });
  }, [jumpToPrevError]);
  ...
  useEffect(() => {
    ...
    const nextIndex =
      currentWarningIndex < 0
        ? 0
        : (currentWarningIndex + 1) % warningIndices.length;
    setCurrentWarningIndex(nextIndex);
    virtuosoRef.current?.scrollToIndex({
      index: warningIndices[nextIndex],
      align: "center",
      behavior: "smooth",
    });
  }, [jumpToNextWarning]);
  ...
  useEffect(() => {
    ...
    const prevIndex =
      currentWarningIndex <= 0
        ? warningIndices.length - 1
        : currentWarningIndex - 1;
    setCurrentWarningIndex(prevIndex);
    virtuosoRef.current?.scrollToIndex({
      index: warningIndices[prevIndex],
      align: "center",
      behavior: "smooth",
    });
  }, [jumpToPrevWarning]);
  ...
  useEffect(() => {
    ...
    const matchIndex = displayedLogs.findIndex(
      (log) => log.hash === searchCurrentMatchHash,
    );
    if (matchIndex !== -1) {
      virtuosoRef.current?.scrollToIndex({
        index: matchIndex,
  ...
  }, [searchCurrentMatchHash, displayedLogs]);
  ...
  }, [jumpToHash, displayedLogs, onJumpComplete]);

  // Render function for each item
  const itemContent = useCallback(
    (index: number, log: LogEntry) => {
      const prev = index > 0 ? displayedLogs[index - 1] : null;
      const next =
